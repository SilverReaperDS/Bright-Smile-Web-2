...
export async function getTestimonials(opts = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => '');
      throw new Error(`Failed to fetch testimonials.json: ${res.status} ${text}`);
    }
    return res.json();
  }
}
...
function normalizeGalleryCase(row) {
  ...
  const after = row.afterImageUrl ? resolvePublicAssetUrl(row.afterImageUrl) : null;
  return {
    id: row.id,
    title: row.title,
...
function legacyGalleryJsonToCases(items) {
  ...
  for (let i = 0; i < list.length; i += 2) {
    ...
    out.push({
      ...
      description: b?.caption && b.caption !== a.caption ? `After: ${b.caption}` : undefined,
      category: undefined,
...
export async function patchAdminGalleryCase(id, bodyOrForm) {
  ...
  const res = await fetch(`${getApiBase()}/api/gallery/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    headers: isForm ? (token ? { Authorization: `Bearer ${token}` } : {}) : authHeaders(),
...
export async function fetchAdminTestimonials(filters = {}) {
  ...
  if (filters.rating != null && filters.rating !== '' && filters.rating !== 'all') {
    q.set('rating', String(filters.rating));
  }
  const qs = q.toString();
  const res = await fetch(`${getApiBase()}/api/admin/testimonials${qs ? `?${qs}` : ''}`, {
    headers: authHeaders(),
  });
  if (!res.ok) throw new Error(await parseErrorResponse(res));
  return res.json();
...
export async function patchAdminTestimonial(id, body) {
  const res = await fetch(`${getApiBase()}/api/admin/testimonials/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    headers: authHeaders(),
    body: JSON.stringify(body),
  });
